...
import { DEMO_VERIFICATION_REVIEWS, getDemoCompany } from '../../../../src/lib/demo/management-demo-data';
...
export default async function CompanyVerificationPage({ params }: CompanyVerificationPageProps) {
  ...
  const reviews = DEMO_VERIFICATION_REVIEWS.filter((review) => review.targetSummary.targetPublicId === company.companyPublicId);

  return (
    <ManagementDetailPage
      description="Company verification reviews with target, risk, priority, and assignment context."
      links={[{ label: 'Company overview', href: `/companies/${company.companyPublicId}` }, { label: 'Members', href: `/companies/${company.companyPublicId}/members` }, { label: 'Documents', href: `/companies/${company.companyPublicId}/documents` }, { label: 'All verification reviews', href: '/verification-reviews' }]}
      ...
      statusTone={company.verificationStatus === 'approved' ? 'success' : company.verificationStatus === 'flagged' ? 'danger' : 'warning'}
      summary="Verification records are connected to the company public ID and use the global verification review contract."
      title="Company verification"
    >
      <VerificationReviewsTable reviews={reviews.length > 0 ? reviews : DEMO_VERIFICATION_REVIEWS.slice(0, 1)} />
    </ManagementDetailPage>
  );
}
